...
A implementação foi organizada de forma modular para facilitar a comparação entre as três versões solicitadas: sequencial, \textit{std::thread} e \textit{OpenMP}. A lógica principal do algoritmo foi mantida em uma estrutura comum, enquanto cada versão altera apenas a forma de executar a ordenação dos blocos.

\indent Essa organização evita duplicação de código e garante que as três versões executem a mesma sequência de etapas: divisão dinâmica do vetor em blocos de tamanho 4 ou 5, ordenação dos blocos pares e ímpares com algoritmos definidos pela combinação escolhida, consolidação final dos blocos e validação do resultado.

\indent Também foram mantidos os mesmos vetores de entrada e as mesmas sementes de particionamento nas três versões. Essa decisão torna a comparação mais justa, pois reduz a interferência de diferenças nos dados de entrada ou na formação dos blocos.

\indent Além disso, foram adicionadas validações para verificar se os blocos possuem tamanho correto, se o vetor final está ordenado e se todos os elementos originais foram preservados. Os resultados também são exportados em arquivos auxiliares, permitindo a geração dos gráficos utilizados na análise.

\indent Por fim, a implementação inclui uma etapa automática de escolha da melhor combinação de algoritmos. Em vez de definir manualmente uma combinação fixa, o programa avalia as combinações permitidas pelo enunciado, respeitando a regra de não repetir o mesmo algoritmo em etapas diferentes. A combinação com menor tempo médio é então usada nas execuções comparativas.
...
\begin{itemize}
    ...
    \item Biblioteca OpenMP: \textit{libomp}
    
\end{itemize}

\indent Como o desenvolvimento foi feito em macOS, foi necessário configurar explicitamente a biblioteca \textit{libomp} no \textit{Makefile}. Em ambientes Linux, a compilação com OpenMP normalmente pode ser feita apenas com a flag \texttt{-fopenmp}. No macOS, porém, o compilador não encontra automaticamente os cabeçalhos e a biblioteca de linkedição do OpenMP. Por isso, o \textit{Makefile} identifica o sistema como \textit{Darwin} e adiciona os caminhos de inclusão e linkedição da \textit{libomp}, instalada via Homebrew.
