...
int main() {
    ...
    vector<string> algos = {"bubble_sort", "insertion_sort", "merge_sort", "quick_sort", "selection_sort"};
    
    // Summary data for gnuplot: Size | Bubble | Insertion | Merge | Quick | Selection
    ofstream summaryFile("results/averages.dat");
    summaryFile << "# Size bubble_sort insertion_sort merge_sort quick_sort selection_sort" << endl;
    ...
    for (int size : sizes) {
        ...
        // Generate 30 vectors once for this size to be fair
        ...
        for (const string& algo : algos) {
            ...
            for (int i = 0; i < iterations; i++) {
                vector<int> data = base_vectors[i]; // Copy the base vector
                Timer t;
                
                if (algo == "bubble_sort") { BubbleSort().sort(data); }
                else if (algo == "insertion_sort") { InsertionSort().sort(data); }
                else if (algo == "merge_sort") { MergeSort().sort(data); }
                else if (algo == "quick_sort") { QuickSort().sort(data); }
                else if (algo == "selection_sort") { SelectionSort().sort(data); }

                double elapsed = t.stop()[0]; // Duration Chrono
                total_time += elapsed;
